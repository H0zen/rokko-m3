        ...
        void MovementInform(Motion::Kind uiMotionType, uint32 uiPointId) override
        {
            ...
            if (uiPointId == POINT_ID_CENTER)
            {
                if (m_uiPhase == PHASE_5_WAITING)
                {
                    ...
                    m_creature->Movement()->GoTo(POINT_ID_AIR, m_creature->Where().X(), m_creature->Where().Y(), m_creature->Where().Z() + 30.0f, false);
                    m_uiPhaseTimer = 0;
                ...
                case PHASE_7_GRAVITY:
                    ...
                    if (m_uiPhase == PHASE_4_SOLO)
                    {
                        if (m_creature->GetHealthPercent() < 50.0f)
                        {
                            ...
                            m_creature->Movement()->StopAndDefault();
                            m_creature->Movement()->GoTo(POINT_ID_CENTER, aCenterPos[0], aCenterPos[1], aCenterPos[2]);
