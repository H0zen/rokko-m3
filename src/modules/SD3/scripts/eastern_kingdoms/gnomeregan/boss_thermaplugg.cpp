...
struct boss_thermaplugg : public CreatureScript
{
    ...
    struct boss_thermapluggAI : public ScriptedAI
    {
        ...
        void GetAIInformation(ChatHandler& reader) override
        {
            ...
            // m_asBombFaces is a fixed-size array member, so the `if (m_asBombFaces)` that
            // used to guard this loop tested the address of that array and was always true.
        ...
        void UpdateAI(const uint32 uiDiff) override
        {
            ...
            // Spawn bombs. The `if (m_asBombFaces)` that used to wrap this loop tested the
            // address of a fixed-size array member and was always true.
            for (uint8 i = 0; i < MAX_GNOME_FACES; ++i)
            {
