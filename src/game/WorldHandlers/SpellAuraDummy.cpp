                ...
                switch (GetId())
                {
                    ...
                    case 10255:                             // Stoned
                    {
                        if (Unit* caster = GetCaster())
                        {
                            ...
                            caster->Movement()->Forbid(Motion::Inhibition::Rooted, Motion::ControlClaim(10255, 0, caster->GetObjectGuid().GetCounter()));
                        }
                        return;
                    }
                    ...
                    case 51405:                             // Digging for Treasure
                        ...
                        target->Movement()->Forbid(Motion::Inhibition::Stunned, Motion::ControlClaim(51405, 0, GetCasterGuid().GetCounter()));
                        return;
    ...
    else
    {
        ...
        switch (GetId())
        {
            ...
            case 42517:                                     // Beam to Zelfrax
            {
                ...
                if (pSummon && pCaster)
                {
                    pSummon->Movement()->GoTo(0, pCaster->Where().X(), pCaster->Where().Y(), pCaster->Where().Z());
                }
            ...
            case 51405:                                     // Digging for Treasure
            {
                ...
                target->Movement()->Allow(Motion::Inhibition::Stunned, Motion::ControlClaim(51405, 0, GetCasterGuid().GetCounter()));
                return;
            }
            ...
            switch (GetId())
            {
                case 6606:                                  // Self Visual - Sleep Until Cancelled (DND)
                {
                    if (apply)
                    {
                        ...
                        target->Movement()->Forbid(Motion::Inhibition::Rooted, Motion::ControlClaim(6606, 0, GetCasterGuid().GetCounter()));
                    }
                    else
                    {
                        target->Movement()->Allow(Motion::Inhibition::Rooted, Motion::ControlClaim(6606, 0, GetCasterGuid().GetCounter()));
                ...
                case 51329:                                 // Feign Death
                {
                    if (target->GetTypeId() == TYPEID_UNIT)
                    {
                        ...
                        if (apply)
                        {
                            ...
                            target->Movement()->Forbid(Motion::Inhibition::Dead, Motion::ControlClaim(GetId(), 0, GetCasterGuid().GetCounter()));
                        }
                        else
                        {
                            ...
                            target->Movement()->Allow(Motion::Inhibition::Dead, Motion::ControlClaim(GetId(), 0, GetCasterGuid().GetCounter()));
                        }
                    }
                    return;
                }
                ...
                case 43874:                                 // Scourge Mur'gul Camp: Force Shield Arcane Purple x3
                    ...
                    if (apply)
                    {
                        target->Movement()->Forbid(Motion::Inhibition::Rooted, Motion::ControlClaim(43874, 0, GetCasterGuid().GetCounter()));
                    }
                    else
                    {
                        target->Movement()->Allow(Motion::Inhibition::Rooted, Motion::ControlClaim(43874, 0, GetCasterGuid().GetCounter()));
                    }
                    return;
